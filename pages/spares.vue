        ...
        <div v-if="addSpare">
            <form  @submit.prevent  enctype="multipart/form-data">
                ...
                <div class="flex justify-between w-72">
                    ...
                    <button @click="addSpare=false; dialogError=null; cleardata()" class="bg-indigo-950 text-white py-2 px-3 rounded-full">Cancel</button>
                </div>
            </form>
        </div>
...
const { data, status,pending, error, refresh, clear } = await useFetch('http://165.22.123.187/api/spares',
    {
        method:'GET',
...
async function createSpare() {
    ...
    const data = await $fetch('http://165.22.123.187/api/spares',
        {
            method:'POST',
            body:params,
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
...
async function deleteSpare() {
    ...
    const data = await useFetch('http://165.22.123.187/api/spares/'+id,
        {
            method:'DELETE',
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
            onResponse({response}){
...
async function updateSpare() {
    ...
    const data = await $fetch('http://165.22.123.187/api/spares/'+popItemId.value,
        {
            method:'PATCH',
            body:params,
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
