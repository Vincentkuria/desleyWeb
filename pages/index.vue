...
const {data, pending, error, refresh, clear }= await useFetch('http://127.0.0.1:8000/api/customers',
        {
            method:'GET',
...
async function logMeOut() {
    ...
    const data=await $fetch('http://127.0.0.1:8000/api/elogout',
        {
            method:'POST',
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
...
if (searchQuery.value) {
    const data= await $fetch('http://127.0.0.1:8000/api/customers-search',
        {
            method:'GET',
...
async function deleteCustomer(id) {
    ...
    const data= await $fetch('http://127.0.0.1:8000/api/customers/'+id,
    {
        method:'DELETE',
        headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
        onResponse({response}){
...
async function changePassword() {
    ...
    if (confirmPassword.value!=null && newPassword.value!=null) {
        ...
        const data = await $fetch('http://127.0.0.1:8000/api/update-cus-password',
            {
                method:'POST',
                headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
                params:{
...
async function updateCustomer() {
    ...
    const data= await $fetch('http://127.0.0.1:8000/api/customers/'+popItemId.value,
        {
            method:'PATCH',
            params:params,
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
