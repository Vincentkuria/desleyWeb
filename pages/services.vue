        ...
        <div class="h-1/5">
            ...
            <div class="w-full bg-indigo-950 text-white h-2/3 flex  justify-items-center ">
                <ul class="flex m-auto">
                    <li class="w-28 text-center p-2">Service</li>
        ...
        <div class="h-4/5 overflow-scroll">
            ...
            <div v-else v-for="(item,index) in myData" class="w-full p-3 flex  justify-items-center shadow-lg m-2 ">
                <ul class="flex m-auto">
                    ...
                    <li class="w-52 h-24 text-center py-1 px-2 overflow-y-scroll">{{ item.service_description }}</li>
...
const { data, status,pending, error, refresh, clear } = await useFetch('http://138.68.154.175/api/services',
    {
        method:'GET',
...
async function createEquipment() {
    ...
    const data = await $fetch('http://138.68.154.175/api/services',
        {
            method:'POST',
            body:params,
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
...
async function deleteEquipment(id) {
    ...
    const data = await useFetch('http://138.68.154.175/api/services/'+id,
        {
            method:'DELETE',
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
            onResponse({response}){
                if (response.status==200) {
                    refresh();
                    snackbarNote.value='service deleted successfully';
                    snackbarSuccess.value=true;
                    snackbarOpen.value=true;
                }
            }
...
async function updateEquipment() {
    ...
    params.append('service_description',updateDescription.value);
    params.append('price',updatePrice.value);
    params.append('img',updateImg.value);

        
    const data = await $fetch('http://138.68.154.175/api/services/'+popItemId.value,
        {
            method:'PATCH',
            body:params,
            headers:{Accept:'application/vnd.api+json',Authorization:'Bearer '+localStorage.getItem('token')},
